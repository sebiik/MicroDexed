...
AudioMixer4              dlyFbMixer;         //xy=584,134
AudioEffectDelay         delay1;         //xy=598,313
AudioOutputI2S           i2s1;           //xy=799,289
AudioConnection          patchCord1(playSdWav1, 0, filter1, 0);
AudioConnection          patchCord2(filter1, 1, dlyFbMixer, 0); // filter 1, 1 is bandbass -
// try filter 1, 0 for high pass
// try filter 1, 2 for high pass
AudioConnection          patchCord3(dlyFbMixer, delay1);
AudioConnection          patchCord4(delay1, 0, dlyFbMixer, 1);
...
void setup() {
  ...
  dlyFbMixer.gain(0, 0.7);
  dlyFbMixer.gain(1, 0.7);
  delay1.delay(0, 400);
...
void loop() {
  ...
  /*
    button0.update();
    if (button0.read() == LOW) {
      dlyFbMixer.gain(1, feedback * 2.0);
    }
